                                            ...
                                            int silent, uct_ib_mem_t *memh)
{
    ...
    memh->mr = UCS_PROFILE_CALL(ibv_reg_mr, md->pd, address, length,
    ...
    ucs_debug("registered memory %p..%p on %s lkey 0x%x rkey 0x%x flags 0x%x",
              address, address + length, uct_ib_device_name(&md->dev),
              memh->mr->lkey, memh->mr->rkey, flags);
                                   ...
                                   unsigned flags, uct_mem_h *memh_p)
{
    ...
    memh = ucs_calloc(1, sizeof(uct_ib_mem_t), "ib_memh");
    if (memh == NULL) {
...
static ucs_status_t uct_ib_mem_dereg(uct_md_h uct_md, uct_mem_h memh)
{
    ...
    status = uct_ib_dereg_mr(ib_memh->mr);
    ucs_free(ib_memh);
    return status;
}
                                     ...
                                     void *rkey_buffer)
{
    ...
    uct_ib_md_pack_rkey(memh->mr->rkey, UCT_IB_INVALID_RKEY, rkey_buffer);
    return UCS_OK;
}
                                       ...
                                       ucs_rcache_region_t *rregion)
{
    ...
    (void)uct_ib_dereg_mr(region->memh.mr);
}
                                         ...
                                         size_t max)
{
    ...
    snprintf(buf, max, "lkey 0x%x rkey 0x%x atomic_rkey 0x%x",
             memh->mr->lkey, memh->mr->rkey, UCT_IB_INVALID_RKEY);
}
                                   ...
                                   ucs_rcache_ops_t *rc_ops)
{
    ...
    rcache_params.region_struct_size = sizeof(ucs_rcache_region_t) + memh_struct_size;
                            ...
                            const uct_ib_md_config_t *md_config)
{
    ...
    for (i = 0; i < md_config->reg_methods.count; ++i) {
        if (!strcasecmp(md_config->reg_methods.rmtd[i], "rcache")) {
            status = uct_ib_md_init_rcache(md, md_attr, md_config,
                                           sizeof(uct_ib_mem_t),
                                           &uct_ib_md_rcache_ops,
                                           &uct_ib_rcache_ops);
            if (status == UCS_OK) {
                return UCS_OK;
            }
...
uct_ib_md_open(const char *md_name, const uct_md_config_t *uct_md_config, uct_md_h *md_p)
{
    ...
    status = md->ops->init_priv(md, &md_attr, md_config);
    if (status != UCS_OK) {
        goto err_dev_cfg;
    }
                                         ...
                                         uct_ib_md_t **p_md)
{
    ...
    md->dev.flags &= ~UCT_IB_DEVICE_FLAG_MLX5_PRM;
    *p_md = md;
    return UCS_OK;
...
static ucs_status_t
uct_ib_verbs_md_init(uct_ib_md_t *md, uct_md_attr_t *md_attr,
                     const uct_ib_md_config_t *md_config)
{
    ucs_status_t status;
...
void uct_ib_verbs_md_cleanup(uct_ib_md_t *md)
{
    if (md->rcache != NULL) {
...
static uct_ib_md_ops_t uct_ib_verbs_md_ops = {
    ...
    .init_priv         = uct_ib_verbs_md_init,
    .cleanup_priv      = uct_ib_verbs_md_cleanup,
};
