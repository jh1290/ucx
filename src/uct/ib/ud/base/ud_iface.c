...
uct_ud_iface_create_qp(uct_ud_iface_t *self, const uct_ud_iface_config_t *config)
{
    ...
    pd = uct_ib_iface_md(&self->super)->pd;
#if HAVE_DECL_IBV_ALLOC_TD
    if (self->super.res_domain != NULL) {
        pd = self->super.res_domain->ibv_domain;
    }
#endif
    /* Create QP */
    memset(&qp_init_attr, 0, sizeof(qp_init_attr));
    qp_init_attr.qp_context          = NULL;
    qp_init_attr.send_cq             = self->super.cq[UCT_IB_TX];
    qp_init_attr.recv_cq             = self->super.cq[UCT_IB_RX];
    ...
    self->qp = ibv_exp_create_qp(uct_ib_iface_device(&self->super)->ibv_context,
                                 &qp_init_attr);
#else
    self->qp = ibv_exp_create_qp(pd, &qp_init_attr);
#endif
    if (self->qp == NULL) {
...
UCS_CLASS_INIT_FUNC(uct_ud_iface_t, uct_ud_iface_ops_t *ops, uct_md_h md,
                    ...
                    int flags, const uct_ud_iface_config_t *config)
{
    ...
    UCS_CLASS_CALL_SUPER_INIT(uct_ib_iface_t, &ops->super, md, worker,
                              ...
                              res_domain_key, flags, &config->super);
...
ucs_status_t uct_ud_iface_event_arm(uct_iface_h tl_iface, unsigned events)
{
    ...
    if (events & (UCT_EVENT_SEND_COMP | UCT_EVENT_RECV)) {
        ...
        status = iface->super.ops->arm_cq(&iface->super, UCT_IB_RX, 0);
        if (status != UCS_OK) {
            goto out;
        }
    }
