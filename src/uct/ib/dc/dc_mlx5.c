...
void uct_dc_mlx5_destroy_dct(uct_dc_mlx5_iface_t *iface)
{
    if (iface->rx.dct != NULL) {
        ...
        mlx5dv_devx_obj_destroy(iface->rx.dct);
#else
        ibv_destroy_qp(iface->rx.dct);
#endif
        iface->rx.dct = NULL;
...
{
#if HAVE_DEVX
    uct_ib_mlx5_md_t *md = ucs_derived_of(uct_ib_iface_md(&iface->super.super.super), uct_ib_mlx5_md_t);
    uct_ib_device_t *dev = &md->super.dev;
    ...
    iface->rx.srq_db = ucs_mpool_get_inline(&md->dbrec_pool);
    if (!iface->rx.srq_db) {
        goto err_free_mem;
    }
    iface->super.rx.srq.db = &iface->rx.srq_db->db[MLX5_RCV_DBR];
    ...
    UCT_IB_MLX5DV_SET(wq, wq, dbr_umem_id,   iface->rx.srq_db->mem_id);
    UCT_IB_MLX5DV_SET64(wq, wq, dbr_addr,    iface->rx.srq_db->offset);
...
err_free:
    ucs_mpool_put_inline(iface->rx.srq_db);
err_free_mem:
...
static UCS_CLASS_CLEANUP_FUNC(uct_dc_mlx5_iface_t)
{
    ...
    if (UCT_RC_MLX5_TM_ENABLED(&self->super)) {
        ...
        ucs_mpool_put_inline(self->rx.srq_db);
