...
ucs_status_t uct_rc_mlx5_get_cmd_qp(uct_rc_mlx5_iface_common_t *iface)
{
    ...
    iface->tm.cmd_wq.qp.verbs.rd = NULL;
    ...
    iface->tm.cmd_wq.qp.type = UCT_IB_MLX5_QP_TYPE_VERBS;
    status = uct_ib_mlx5_iface_init_res_domain(&iface->super.super,
                                               &iface->tm.cmd_wq.qp);
    if (status != UCS_OK) {
        goto err;
    }
    ...
    qp = ibv_create_qp(iface->tm.cmd_wq.qp.verbs.rd->pd, &qp_init_attr);
    if (qp == NULL) {
...
err_destroy_qp:
    ibv_destroy_qp(qp);
err_rd:
    uct_ib_mlx5_iface_cleanup_res_domain(&iface->tm.cmd_wq.qp);
err:
    return UCS_ERR_NO_DEVICE;
qp_ok:
#endif
    iface->tm.cmd_wq.qp.qp_num = qp->qp_num;
...
{
    uct_ib_md_t *md              = uct_ib_iface_md(&iface->super.super);
    unsigned max_cancel_sync_ops = 0;
