                    ...
                    uct_ib_iface_init_attr_t *init_attr)
{
    ...
    UCS_CLASS_CALL_SUPER_INIT(uct_rc_iface_t, &uct_rc_mlx5_iface_ops,
                    ...
                    const uct_iface_config_t *tl_config)
{
    ...
    self->super.tx.mmio_mode               = config->mlx5_common.mmio_mode;
    self->super.tx.bb_max                  = ucs_min(config->tx_max_bb, UINT16_MAX);
    self->super.super.config.tx_moderation = ucs_min(self->super.super.config.tx_moderation,
    ...
    status = uct_rc_mlx5_iface_tag_init(&self->super, config);
    if (status != UCS_OK) {
        return status;
    }
