                                         ...
                                         unsigned max_send_wr)
{
    ...
    if (md->flags & UCT_IB_MLX5_MD_FLAG_DEVX) {
        status = uct_ib_mlx5_txwq_init_devx(iface->super.super.super.worker,
                                            md, qp, txwq,
        ...
        return uct_ib_mlx5dv_create_qp(ib_iface, txwq, NULL, &attr, qp);
    }

    status = uct_ib_mlx5_iface_set_verbs(ib_iface, qp, &attr);
    if (status != UCS_OK) {
        return status;
    }
...
#else
    uct_rc_iface_fill_attr(&iface->super, &attr, max_send_wr);
    status = uct_ib_mlx5_iface_create_qp(ib_iface, qp, &attr, &qp->verbs.qp);
    if (status != UCS_OK) {
        return status;
    }
#endif
